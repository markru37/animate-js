document.addEventListener("DOMContentLoaded", () => {
    ...
    buttons.forEach(button => {
        ...
        buttonContainer.setAttribute("data-attr", button.id + 1);
    ...
    const buttontTransform = document.querySelectorAll('.dot__button');
    const textTransform = document.querySelectorAll('.text__title');
    let currentAngle = 0;
    ...
    buttonContainer.forEach(element => {
        element.addEventListener('click', () => {
            ...
            buttontTransform.forEach(button => button.classList.remove('_active'));


            const clickedButton = element.querySelector('.dot__button');
            clickedButton.classList.add('_active');

            const index = element.getAttribute('data-attr');
            rotateToElement(index);
        });
    });
    function rotateToElement(index) {
        const targetAngle = (index - 1) * 60;
        console.log(index - 1);
        ...
        gsap.to(".test__container", {
            rotation: -currentAngle,
            ease: "power2.inOut",
            onUpdate: fixElementOrientation
        });
    }


    const transferText = (currentAngle) => {
        let x = 0;
        let y = 0;
        switch (currentAngle) {
            case 0:
    ...
    buttontTransform.forEach(button => {
        ...
        button.addEventListener('mouseleave', () => {
            gsap.to(button, {
                scale: button.classList.contains('_active') ? 3 : 1,
                ease: "power2.inOut",
                borderColor: button.classList.contains('_active') ? '#c0c3c8' : 'transparent',
                color: button.classList.contains('_active') ? '#5c6578' : 'transparent',
            });
        });
    });

    function fixElementOrientation() {
        buttontTransform.forEach(element => {
            gsap.to(element, {
                rotation: currentAngle,
                scale: element.classList.contains('_active') ? 3 : 1,
                ease: "power2.inOut",
                zIndex: element.classList.contains('_active') ? 3 : 1
            });
        });

        textTransform.forEach(element => {
            let [x, y] = transferText(currentAngle);
            gsap.to(element, {
                rotation: currentAngle,
